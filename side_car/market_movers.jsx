...
// KPIs we include in the snapshot — subset of KPI_SPECS that are most actionable
const SNAPSHOT_KPIS = [
...
export default function MarketMoversConcise({
    ...
    const [skippedPeers, setSkippedPeers] = useState([]); // peers excluded due to missing data
    const [tape, setTape] = useState("");
    const [focusBankSnapshot, setFocusBankSnapshot] = useState(null); // perspective bank KPI context
    ...
    useEffect(() => {
        ...
        if (computed?.snapshot !== undefined) setFocusBankSnapshot(computed.snapshot);
    }, [computed]);
    ...
    async function onSendToGemini() {
        ...
        try {
            const prompt = buildGeminiPrompt({ tape, perspectiveBankName, focusBankSnapshot });
