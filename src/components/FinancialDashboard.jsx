...
const FinancialDashboard = ({ financials, benchmarks, authRequired = true, isPresentMode, setIsPresentMode, secondaryBank, setSecondaryBank, secondaryFinancials, loadingSecondary }) => {
    ...
    return (
        <div id="dashboard-export-zone" className="space-y-8 overflow-x-hidden w-full">
            <div id="dashboard-header" className="border-b border-slate-800 pb-4 mb-6 space-y-3">
                {/* Row 1: Title + Action Buttons */}
                <div className="flex flex-wrap items-center gap-3">
                    <h2 className="font-extrabold text-white tracking-tight text-2xl shrink-0">
                        Financial Health Scorecard
                    </h2>
                    <div className="flex items-center gap-2 ml-auto flex-wrap">
                        <button
                            onClick={() => setIsSummaryModalOpen(true)}
                            className="flex items-center justify-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-4 py-2 rounded-full text-sm font-bold shadow-md hover:shadow-lg transition-all transform hover:-translate-y-0.5 whitespace-nowrap"
                        >
                ...
                {/* Row 2: Benchmark + Compare indicators — only shown when data is loaded */}
