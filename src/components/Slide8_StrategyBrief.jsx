...
const Slide8_StrategyBrief = ({ dataProvider, segmentKey, segmentLabel, priorQuarter, currentQuarter, focusBankCert, focusBankName, isPresentationMode = true }) => {
    ...
    return (
        <div className="w-full h-full flex flex-col justify-start px-4 pt-2 overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700 fill-mode-both">
            <div className="flex-1 overflow-y-hidden space-y-2 pr-2 pb-2 custom-scrollbar">
                {movers.map((mover, idx) => (
                    <div key={mover.cert} className="bg-white border border-slate-200 p-3 rounded-lg shadow-sm flex items-start gap-4">
                        <div className="w-8 h-8 rounded bg-blue-50 text-blue-800 font-black text-lg flex items-center justify-center flex-shrink-0 border border-blue-100">
                            {idx + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between mb-1">
                                <h3 className="text-lg font-bold text-slate-900 truncate pr-4">{mover.bankName}</h3>
                                <div className="text-right flex-shrink-0">
                                    <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-0.5">Primary Driver</div>
                            ...
                            <div className="bg-slate-50 border-l-4 border-blue-900 p-2.5 rounded-r-md">
