...
const MoversSummaryModal = ({ isOpen, onClose, dataProvider, segmentKey, priorQuarter, currentQuarter, perspectiveBankName, focusBankCert, authRequired = true }) => {
    ...
    const { retryCountdown, retryCount, setRetryFromError, resetRetry } = useRetryCountdown(() => {
        if (generateMoversIntelligence) generateMoversIntelligence();
    });
    ...
    const generateMoversIntelligence = useCallback(async () => {
        setIsLoading(true);
        setError(null);
        try {
            // 1. Fetch Data
    ...
    }, [dataProvider, segmentKey, priorQuarter, currentQuarter, focusBankCert, perspectiveBankName, retryCount, setRetryFromError, resetRetry, user]);
    ...
    }, [isOpen, user, authRequired, summary, isLoading, error, isLoginModalOpen, generateMoversIntelligence]);
